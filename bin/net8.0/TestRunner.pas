...
{$reference '%GAC%\System.Windows.Forms.Primitives.dll'}
{$reference '%GAC%\Accessibility.dll'}

uses PascalABCCompiler, System.IO, System.Diagnostics, LanguageIntegration;
...
begin
  ...
  writeln('Total ' + (files.Length) + ', errors ' + errorCount);
...
begin
  ...
  begin
    ...
    begin
      ...
      //Halt();
      errorCount += 1;
      if comp.ErrorsList[0].ToString().Contains('NotImplemented') then
      begin
        notImplemented += 1;
      end;
      writeln('Error ' + comp.ErrorsList[0].ToString());
    end;
    if i mod 50 = 0 then
    begin
      System.GC.Collect();
  ...
  writeln('Total ' + (files.Length) + ', errors ' + errorCount + ', not implemented ' + notImplemented);
  //Println;
end;
